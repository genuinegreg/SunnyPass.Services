...
angular.module('SunnyPass.Service')
    .factory('Locker', function SunnyPassLockerFactory($log, $q, $timeout, $angularCacheFactory, Crypto, Secret) {

        var DATABASE_SERVER = 'http://server:5984/';

        var encryptionKeyCache = $angularCacheFactory('keyCache', {
            maxAge: 5 * 60 * 1000,
        ...
        Locker.lockAll = function() {
            ...
            encryptionKeyCache.removeAll();
        };

        Locker.prototype.isLocked = function() {
            return !encryptionKeyCache.get(this.secret.key);
        };

        Locker.prototype.lock = function() {
            encryptionKeyCache.remove(this.secret.key);
        };
        ...
        Locker.prototype.unlock = function(password) {
            ...
            _this._checkEncryptionKey(encryptionKey).then(
                function resolved() {
                    ...
                    encryptionKeyCache.put(_this.secret.key, encryptionKey);
        ...
        Locker.prototype._getEncryptionKey = function() {
            ...
            var encryptionKey = encryptionKeyCache.get(_this.secret.key);
         ...
         * @returns {Promise.promise|*}
         */
        Locker.prototype.save = function(meta, data) {
