...
//            $log.debug('PouchDB locker instance connected on', dbUrl);
        ...
        /**
         * Check is this locker is locked
         * @returns {boolean}
         */
        Locker.prototype.isLocked = function () {
            var keyValue = KeyCache.lookup(this.secret.key)
            return !keyValue || keyValue.expired;
        };
        ...
        /**
         * Wipe database and lock everything
         * @type {*}
         */
        Locker.prototype.wipe = function () {
            var d = $q.defer();
            ...
            // destroy db
            PouchDB.destroy(this.secret.shared, function (err) {
                if (err) {
                    d.reject(err);
                }
                d.resolve();
         ...
         * @returns {Promise.promise|*} promise reloved when all metadata are loaded
         */
         ...
         * @param key key
         * @param value value
         * @returns {Promise.promise|*} resolved when saved
         */
        ...
        Locker.prototype.list = function () {
            ...
            if (!encryptionKey) {
                d.reject(new Error('locked'));
                return d.promise;
            }


            /**
             * filter and decrypt db.allDocs results
             * @param list db.allDocs results
             * @returns {*} array with passwords and undefined values
             */
            function filterAndDecryptList(list) {
                var filterAndDecryptListDefered = $q.defer();
                ...
                async.map(
                    ...
                    function iterator(item, cb) {
                        ...
                        if (!item.doc || item.doc.type !== 'data') {
                            // if not 'data' return undefined
                            return cb();
                        }
                        ...
                        try {
                            ...
                            // add data flags
                            var data = JSON.parse(Crypto.decrypt(encryptionKey, doc.encryptedData));
                            doc.meta.password = !!data.password;
                            doc.meta.login = !!data.login;
                            doc.meta.notes = !!data.notes;
                    ...
                    function callback(err, list) {
                        if (err) {
                            filterAndDecryptListDefered.reject(err);
                        }
                        $log.debug('Locker.list()... OK');
                        filterAndDecryptListDefered.resolve(list);
                    });

                return filterAndDecryptListDefered.promise;
            ...
            function filterUndefined(list) {
                var filterDefered = $q.defer();

                async.filter(list,
                    function filterFunc(item, cb) {
                        cb(!!item);
                    },
                    function callback(results) {
                        filterDefered.resolve(results);
                    })


                return filterDefered.promise;
            }


            _this.db.allDocs({include_docs: true}, function (err, docs) {

                if (err) {
                    return d.reject(err);
                }

                filterAndDecryptList(docs.rows)
